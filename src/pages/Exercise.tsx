...
import { Calendar as CalendarIcon, Loader2, Plus, Minus, ChevronLeft, ChevronRight, Trash2, Pencil, X, WifiOff, CloudOff, Footprints, Save, Dumbbell, Mountain, PersonStanding, Bike, Disc3, Flower2, CircleDot, Waves, Feather, Target, Timer, Sparkles, Droplets, TreeDeciduous, MoreHorizontal, Camera, Image as ImageIcon, type LucideIcon } from "lucide-react";
...
const SPORT_TYPES = [{
  ...
  value: "hiking",
  label: "등산",
  placeholder: "예: 금정산(왕복)",
  color: "bg-green-600/20 text-green-700 dark:text-green-300 border-green-600/30"
}, {
  value: "running",
  label: "러닝/조깅",
  placeholder: "예: 5km",
  ...
  value: "pilates",
  label: "필라테스",
  placeholder: "예: 기구 필라테스",
  color: "bg-pink-500/20 text-pink-700 dark:text-pink-300 border-pink-500/30"
}, {
  value: "swimming",
  label: "수영",
  placeholder: "예: 자유형 500m",
  color: "bg-sky-500/20 text-sky-700 dark:text-sky-300 border-sky-500/30"
}, {
  value: "badminton",
  label: "배드민턴",
  placeholder: "예: 복식 경기",
  color: "bg-lime-500/20 text-lime-700 dark:text-lime-300 border-lime-500/30"
}, {
  value: "soccer",
  label: "축구",
  placeholder: "예: 조기축구 3경기",
  color: "bg-green-500/20 text-green-700 dark:text-green-300 border-green-500/30"
}, {
  value: "futsal",
  label: "풋살",
  placeholder: "예: 5대5 매치",
  color: "bg-teal-500/20 text-teal-700 dark:text-teal-300 border-teal-500/30"
}, {
  value: "golf",
  label: "골프/파크골프",
  placeholder: "예: 스크린 골프 18홀",
  ...
  value: "gymnastics",
  label: "체조/맨손운동",
  placeholder: "예: 실버체조 또는 스트레칭",
  color: "bg-violet-500/20 text-violet-700 dark:text-violet-300 border-violet-500/30"
}, {
  value: "aquarobics",
  label: "아쿠아로빅",
  placeholder: "예: 수중 에어로빉",
  color: "bg-blue-400/20 text-blue-600 dark:text-blue-300 border-blue-400/30"
}, {
  value: "barefoot",
  label: "맨발 걷기",
  placeholder: "예: 황톳길 맨발 걷기",
  color: "bg-yellow-600/20 text-yellow-700 dark:text-yellow-300 border-yellow-600/30"
}, {
  value: "other",
  label: "기타",
  placeholder: "예: 운동 내용",
...
// Mock 머신명 후보 (헬스용)
const MACHINE_SUGGESTIONS = ["레그 프레스", "레그 익스텐션", "레그 컬", "체스트 프레스", "숄더 프레스", "랫풀다운", "시티드 로우", "케이블 크로스오버", "스미스 머신", "덤벨 벤치 프레스"];
...
function parseExerciseName(name: string): {
  sportType: string;
  sportLabel: string;
  exerciseNames: string[];
} {
  ...
  const exerciseNames = exerciseNamesStr ? exerciseNamesStr.split(",").map(n => n.trim()).filter(Boolean) : [];
  return {
    sportType,
    sportLabel,
    exerciseNames
...
// 운동명 요약 생성 (1줄 + 외 n)
const getExerciseSummary = (names: string[]): {
  line1: string;
  overflow: number;
} => {
...
const ExerciseCard = memo(function ExerciseCard({
  ...
  if (isPhoto) {
    ...
    return <div className="bg-card rounded-2xl border border-border p-3 cursor-pointer hover:bg-muted/50 transition-colors h-24 flex flex-col justify-between relative" onClick={onClick} onMouseEnter={handlePrefetch} onTouchStart={handlePrefetch}>
        {/* 상단: 카메라 아이콘 + 사진기록 (또는 제목) */}
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 min-w-0">
            <Camera className="w-4 h-4 shrink-0" style={{
            color: '#FFA500'
          }} />
  ...
  return <div className="bg-card rounded-2xl border border-border p-3 cursor-pointer hover:bg-muted/50 transition-colors h-24 flex flex-col justify-between relative" onClick={onClick}>
      {/* 상단: 아이콘 + 종목 + 시간 */}
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
...
export default function Exercise() {
  ...
  } = useGymMonthHeaders(monthStr);
  const {
    record: todayGymRecord,
  ...
  } = useGymDayRecord(dateStr);
  const {
    pendingCount,
    isSyncing: pendingSyncing,
    addToPending,
    syncPending
  } = usePendingQueueOptimized();
  ...
  useEffect(() => {
    const handleOnline = async () => {
      ...
      toast({
        title: "온라인 복귀",
        description: "데이터를 동기화합니다."
      });
      const result = await syncPending();
      if (result.success > 0) {
        toast({
          title: "동기화 완료",
          description: `${result.success}개 기록이 서버에 업로드되었습니다.`
        });
        refetchHeaders();
        refetchRecord();
      }
  ...
  const getRecordTypeOnDate = (date: Date): {
    hasNormal: boolean;
    hasPhoto: boolean;
  } => {
  ...
  const handleSaveExercise = async () => {
    ...
    // 운동명들을 쉼표로 조합
    const exerciseNamesStr = addedExerciseNames.length > 0 ? addedExerciseNames.join(", ") : "";
    const displayName = exerciseNamesStr ? `[${sportLabel}] ${exerciseNamesStr}` : `[${sportLabel}]`;
    ...
    try {
      if (editingPendingIndex !== null) {
        ...
        setPendingExercises(prev => prev.map((ex, idx) => idx === editingPendingIndex ? updatedExercise : ex));
        ...
        const newExercises = todayGymRecord.exercises.map(ex => ex.id === editingExerciseId ? exerciseToSave : ex);
        ...
        if (isOnline) {
          ...
          const localId = await addToPending('gym_record', {
            user_id: user.id,
            date: dateStr,
            exercises: todayGymRecord ? [...todayGymRecord.exercises, exerciseToSave] : [exerciseToSave]
          });
          addOffline(todayGymRecord ? [...todayGymRecord.exercises, exerciseToSave] : [exerciseToSave], localId);
          toast({
            title: "로컬에 저장됨",
  ...
  const saveAllExercises = async () => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        toast({
          title: `${exercisesToSave.length}개 운동 기록 저장 완료!`
        });
        refetchHeaders();
      } else {
        const localId = await addToPending('gym_record', {
          user_id: user.id,
          date: dateStr,
          exercises: todayGymRecord ? [...todayGymRecord.exercises, ...exercisesToSave] : exercisesToSave
        });
        addOffline(todayGymRecord ? [...todayGymRecord.exercises, ...exercisesToSave] : exercisesToSave, localId);
        toast({
          title: "로컬에 저장됨",
          description: "온라인 복귀 시 자동으로 서버에 업로드됩니다."
        });
  ...
  const saveExistingExercise = async (exerciseToSave: ExerciseRecord) => {
    ...
    try {
      ...
      const newExercises = todayGymRecord.exercises.map(ex => ex.id === editingExerciseId ? gymExercise : ex);
      await update(todayGymRecord.id, newExercises);
      toast({
        title: "운동 기록 수정 완료!"
      });
  ...
  const toggleDetailEditMode = () => {
    ...
    const {
      exerciseNames
    } = parseExerciseName(detailExercise.name);
    setEditDetailExerciseNames(exerciseNames);
    setEditDetailNameInput("");
  ...
  const saveDetailEdit = async () => {
    ...
    const exerciseNamesStr = editDetailExerciseNames.length > 0 ? editDetailExerciseNames.join(", ") : "";
    const displayName = exerciseNamesStr ? `[${sportLabel}] ${exerciseNamesStr}` : `[${sportLabel}]`;
    ...
    try {
      const newExercises = todayGymRecord.exercises.map(ex => ex.id === detailExercise.id ? updatedExercise : ex);
      await update(todayGymRecord.id, newExercises);
      toast({
        title: "수정 완료!"
      });
  ...
  const saveQuickAdd = async () => {
    ...
    try {
      ...
      for (const base64 of quickAddImages) {
        const {
          originalPath
        } = await uploadFromBase64('gym-photos', user.id, base64, true);
      ...
      if (isOnline) {
        ...
        const localId = await addToPending('gym_record', {
          user_id: user.id,
          date: dateStr,
          exercises: todayGymRecord ? [...todayGymRecord.exercises, photoExercise] : [photoExercise]
        });
        addOffline(todayGymRecord ? [...todayGymRecord.exercises, photoExercise] : [photoExercise], localId);
        toast({
          title: "로컬에 저장됨",
          description: "온라인 복귀 시 자동으로 서버에 업로드됩니다."
        });
  ...
  const savePhotoEdit = async () => {
    if (!detailExercise || !todayGymRecord) return;
    const displayName = editPhotoTitle.trim() ? `[사진기록] ${editPhotoTitle.trim()}` : "[사진기록]";
    const updatedExercise: GymExercise = {
      ...detailExercise,
      name: displayName,
  ...
  return <div className="space-y-6 pb-8">
      ...
      <PageHeader title="운동양갱" subtitle="오늘의 운동을 기록하세요" />

      {/* 오프라인 배너 */}
      {!isOnline && <div className="bg-yellow-100 dark:bg-yellow-900 border border-yellow-300 dark:border-yellow-700 rounded-xl p-3 flex items-center gap-2">
          <WifiOff className="w-5 h-5 text-yellow-600" />
          <span className="text-sm text-yellow-800 dark:text-yellow-200">오프라인 모드 - 데이터가 로컬에 임시 저장됩니다</span>
        </div>}

      {/* 대기 중인 업로드 표시 */}
      {pendingCount > 0 && <div className="bg-blue-100 dark:bg-blue-900 border border-blue-300 dark:border-blue-700 rounded-xl p-3 flex items-center justify-between">
          ...
          {isOnline && !pendingSyncing && <Button variant="ghost" size="sm" onClick={syncPending}>
              지금 동기화
            </Button>}
        </div>}

      {/* 동기화 중 표시 */}
      {(syncing || pendingSyncing) && <div className="flex items-center gap-2 text-sm text-muted-foreground">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between bg-card rounded-2xl border border-border p-4">
          ...
          <button onClick={() => setShowCalendar(true)} className="flex items-center gap-2 text-lg font-semibold whitespace-nowrap">
            <CalendarIcon className="w-5 h-5 flex-shrink-0" />
            <span>{formatDateDisplay(selectedDate)}</span>
            {isToday && <span className="text-xs bg-primary text-primary-foreground px-2 py-0.5 rounded-full flex-shrink-0">
                오늘
              </span>}
          </button>
          <Button variant="ghost" size="icon" onClick={() => moveDate(1)}>
        ...
        <Dialog open={showCalendar} onOpenChange={setShowCalendar}>
          <DialogContent className="max-w-sm">
            ...
            hasRecord: "font-bold"
          }} className="rounded-md" components={{
            DayContent: ({
              date
            }) => {
              const {
                hasNormal,
                hasPhoto
              } = getRecordTypeOnDate(date);
              const isSelected = selectedDate && isSameDay(date, selectedDate);
              const isTodayDate = isSameDay(date, new Date());
              return <div className="flex flex-col items-center justify-center w-full h-full">
                      <span className={cn("text-sm", isTodayDate && "font-bold", isSelected && "text-primary-foreground")}>
                        {date.getDate()}
                      </span>
                      {(hasNormal || hasPhoto) && <div className="flex gap-0.5 text-[8px] leading-none mt-0.5">
        ...
        {/* 장바구니 (추가 대기 중인 운동들) */}
        {pendingExercises.length > 0 && <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 space-y-3">
            ...
            return <div key={exercise.id} className="bg-card rounded-xl p-3">
                    <div className="flex items-center justify-between">
        ...
        {/* 운동 추가 버튼 + 빠른 추가 버튼 */}
        <div className="flex gap-2">
          <Button variant="default" className="flex-1 h-14 text-white" onClick={startNewExercise}>
            <Plus className="w-5 h-5 mr-2" />
            운동 추가
          </Button>
          <Button variant="outline" className="h-14 w-20 flex items-center justify-center bg-white hover:bg-gray-50 border-2 border-primary" onClick={startQuickAdd} aria-label="빠른 추가">
            <Camera className="w-7 h-7 text-primary" />
          </Button>
        </div>

        {/* 빠른 추가용 숨김 파일 입력 */}
        <input ref={quickAddFileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handleQuickAddFileSelect} />
        ...
        <Dialog open={showQuickAdd} onOpenChange={open => !open && cancelQuickAdd()}>
          <DialogContent className="max-w-md p-0 flex flex-col max-h-[85vh] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {quickAddImages.length > 0 ? <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    사진 {quickAddImages.length}장이 선택되었습니다
                  </p>
                  <div className="grid grid-cols-3 gap-2">
                    {quickAddImages.map((img, idx) => <div key={idx} className="relative aspect-square">
                        <img src={img} alt={`사진 ${idx + 1}`} className="w-full h-full object-cover rounded-lg" />
                        <button onClick={() => removeQuickAddImage(idx)} className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-destructive text-destructive-foreground rounded-full flex items-center justify-center shadow-md">
                          <X className="w-3 h-3" />
                        </button>
                      </div>)}
                    {/* 사진 추가 버튼 */}
                    <button onClick={addMoreQuickAddImages} className="aspect-square border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center text-muted-foreground hover:bg-muted/50 transition-colors">
                      <Plus className="w-6 h-6" />
                      <span className="text-xs mt-1">추가</span>
                    </button>
                  </div>
                </div> : <div className="py-12 text-center text-muted-foreground">
                  <Camera className="w-12 h-12 mx-auto mb-3 opacity-50" style={{
                color: '#FFA500'
              }} />
                  <p>사진을 선택해주세요</p>
                </div>}
            ...
            <div className="shrink-0 border-t bg-background px-6 py-4 pb-safe">
              <div className="flex gap-2">
                <Button variant="outline" className="flex-1" onClick={cancelQuickAdd}>
                  취소
                </Button>
                <Button className="flex-1" onClick={saveQuickAdd} disabled={quickAddImages.length === 0 || isQuickAddSaving}>
                  {isQuickAddSaving ? <>
        ...
        <Dialog open={showAddExercise} onOpenChange={open => !open && cancelExercise()}>
          <DialogContent className="max-w-md p-0 flex flex-col max-h-[85vh] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              {currentExercise && <>
                  ...
                  <div>
                    ...
                    <Select value={currentExercise.sportType} onValueChange={handleSportTypeChange}>
                      <SelectTrigger className="mt-1">
                        <SelectValue placeholder="종목 선택" />
                      </SelectTrigger>
                      <SelectContent>
                        {SPORT_TYPES.map(sport => <SelectItem key={sport.value} value={sport.value}>
                            {sport.label}
                          </SelectItem>)}
                      </SelectContent>
                    </Select>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-1">
                      <Input ref={exerciseNameInputRef} value={exerciseNameInput} onChange={e => setExerciseNameInput(e.target.value)} onKeyDown={e => {
                    if (e.key === "Enter") {
                      e.preventDefault();
                      addExerciseName();
                    }
                  }} placeholder={getCurrentPlaceholder()} className="flex-1" />
                      <Button variant="secondary" size="icon" className="shrink-0 h-10 w-10" onClick={addExerciseName} title="운동명 추가">
                        <Plus className="w-5 h-5" />
                      </Button>
                    </div>
                    
                    {/* 추가된 운동명 목록 */}
                    <ExerciseTagList names={addedExerciseNames} onRemove={removeExerciseName} size="md" className="mt-2" />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <Button variant="outline" size="icon" onClick={() => setCurrentExercise({
                    ...currentExercise,
                    duration: Math.max(0, (currentExercise.duration || 0) - 5)
                  })} disabled={!currentExercise.duration || currentExercise.duration <= 0}>
                        <Minus className="w-4 h-4" />
                      </Button>
                      <Input type="number" min={0} value={currentExercise.duration ?? ""} onChange={e => setCurrentExercise({
                    ...currentExercise,
                    duration: e.target.value ? parseInt(e.target.value) : undefined
                  })} placeholder="0" className="w-20 text-center text-lg font-bold" />
                  ...
                  <div>
                    ...
                    <Textarea value={currentExercise.memo || ""} onChange={e => setCurrentExercise({
                  ...currentExercise,
                  memo: e.target.value
                })} placeholder="운동 메모를 남겨보세요" className="mt-1" rows={2} />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <GymPhotoUpload images={currentExercise.images || []} onImagesChange={images => setCurrentExercise({
                    ...currentExercise,
                    images
                  })} />
                    </div>
            ...
            <div className="shrink-0 border-t bg-background px-6 py-4 pb-safe space-y-2">
              ...
              {/* 2줄: 취소 / 저장하기 */}
              <div className="flex gap-2">
                <Button variant="outline" className="flex-1" onClick={cancelExercise}>
                  취소
                </Button>
        ...
        {recordLoading ? <ExerciseSkeleton /> : todayGymRecord && todayGymRecord.exercises.length > 0 ? <div className="space-y-3">
            
            <div className="grid grid-cols-2 gap-3">
              {todayGymRecord.exercises.map(exercise => <ExerciseCard key={exercise.id} exercise={exercise} onClick={() => openDetailSheet(exercise)} />)}
            </div>
          </div> : <div className="bg-muted rounded-2xl p-8 text-center">
      ...
      if (!open) {
        ...
        <SheetContent side="bottom" className={cn("rounded-t-3xl [&>button]:hidden p-0 flex flex-col", isDetailEditMode || isPhotoEditMode ? "h-[85vh]" : "h-auto max-h-[80vh]")}>
          {detailExercise && (() => {
          ...
          if (isPhoto) {
            return <>
                  ...
                  {isPhotoEditMode ? (/* 사진기록 수정 모드 */
              <>
                      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                        {/* 제목 입력 */}
                        <div>
                          <label className="text-sm font-medium text-muted-foreground">제목 (선택)</label>
                          <Input value={editPhotoTitle} onChange={e => setEditPhotoTitle(e.target.value)} placeholder="사진기록 제목을 입력하세요" className="mt-1" />
                        </div>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-3 gap-2 mt-2">
                            {editPhotoImages.map((img, idx) => <div key={idx} className="relative aspect-square">
                                <img src={img} alt={`사진 ${idx + 1}`} className="w-full h-full object-cover rounded-lg" />
                                <button onClick={() => removePhotoEditImage(idx)} className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-destructive text-destructive-foreground rounded-full flex items-center justify-center shadow-md">
                                  <X className="w-3 h-3" />
                                </button>
                              </div>)}
                            {/* 사진 추가 버튼 */}
                            <button onClick={() => photoEditFileInputRef.current?.click()} className="aspect-square border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center text-muted-foreground hover:bg-muted/50 transition-colors">
                              <Plus className="w-6 h-6" />
                              <span className="text-xs mt-1">추가</span>
                            </button>
                          </div>
                        ...
                        {/* 숨김 파일 입력 */}
                        <input ref={photoEditFileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handlePhotoEditFileSelect} />
                      </div>
                      
                      {/* 하단 액션바 - 취소/저장 */}
                      <div className="shrink-0 border-t bg-background px-6 py-4 pb-safe">
                        <div className="flex gap-3">
                          <Button variant="outline" className="flex-1 h-14 text-base" onClick={cancelPhotoEdit}>
                            취소
                          </Button>
                          <Button className="flex-1 h-14 text-base" onClick={savePhotoEdit} disabled={editPhotoImages.length === 0}>
                            저장하기
                          </Button>
                        </div>
                      </div>
              ...
              <>
                      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                        ...
                        {detailExercise.images && detailExercise.images.length > 0 && <GymPhotoUpload images={detailExercise.images} onImagesChange={() => {}} readonly />}
                      </div>
                      
                      {/* 하단 액션바 - 가로 배치 큰 버튼 2개 */}
                      <div className="shrink-0 border-t bg-background px-6 py-4 pb-safe">
                        <div className="flex gap-3">
                          <Button variant="outline" className="flex-1 h-14 text-base font-medium" onClick={startPhotoEdit}>
                            수정하기
                          </Button>
                          <Button variant="destructive" className="flex-1 h-14 text-base font-medium" onClick={() => deleteExercise(detailExercise.id)}>
                            삭제하기
                          </Button>
                        </div>
                      </div>
          ...
          return <>
                ...
                <SheetHeader className="flex flex-row items-center justify-between px-6 pt-6 pb-4 border-b shrink-0">
                  ...
                  <div className="flex items-center gap-1">
                    {!isDetailEditMode && <Button variant="outline" size="sm" onClick={toggleDetailEditMode}>
                ...
                {isDetailEditMode ? (/* 편집 모드 - 스크롤 본문 + sticky 하단 */
            <>
                    {/* 스크롤 영역 */}
                    <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                      {/* 종목 (읽기 전용) */}
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-1">
                          <Input value={editDetailNameInput} onChange={e => setEditDetailNameInput(e.target.value)} onKeyDown={e => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        addDetailExerciseName();
                      }
                    }} placeholder="운동명 입력 후 + 클릭" className="flex-1" />
                          <Button variant="secondary" size="icon" className="shrink-0 h-10 w-10" onClick={addDetailExerciseName}>
                            <Plus className="w-5 h-5" />
                          </Button>
                        </div>
                        {/* 편집 모드 운동명 태그 */}
                        <ExerciseTagList names={editDetailExerciseNames} onRemove={removeDetailExerciseName} size="md" className="mt-2" />
                      </div>
                      ...
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">총 운동시간 (분) - 선택</label>
                        <div className="flex items-center gap-3 mt-1">
                          <Button variant="outline" size="icon" onClick={() => setEditDetailDuration(Math.max(0, (editDetailDuration || 0) - 5))} disabled={!editDetailDuration || editDetailDuration <= 0}>
                            <Minus className="w-4 h-4" />
                          </Button>
                          <Input type="number" min={0} value={editDetailDuration ?? ""} onChange={e => setEditDetailDuration(e.target.value ? parseInt(e.target.value) : undefined)} placeholder="0" className="w-20 text-center text-lg font-bold" />
                          <span className="text-muted-foreground font-medium">분</span>
                          <Button variant="outline" size="icon" onClick={() => setEditDetailDuration((editDetailDuration || 0) + 5)}>
                            <Plus className="w-4 h-4" />
                          </Button>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <Textarea value={editDetailMemo} onChange={e => setEditDetailMemo(e.target.value)} placeholder="메모를 입력하세요" className="mt-1" rows={2} />
                      </div>
                      ...
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">사진</label>
                        <div className="mt-1">
                          <GymPhotoUpload images={editDetailImages} onImagesChange={setEditDetailImages} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="shrink-0 border-t bg-background px-6 py-4 pb-safe space-y-2">
                      <div className="flex gap-2">
                        <Button variant="outline" className="flex-1" onClick={() => setIsDetailEditMode(false)}>
                          취소
                        </Button>
                        <Button className="flex-1" onClick={saveDetailEdit}>
                          저장하기
                        </Button>
                      </div>
                    </div>
            ...
            <>
                    ...
                    {/* 하단 액션바 - sticky 고정 */}
                    <div className="shrink-0 border-t bg-background px-6 py-4 pb-safe">
                      <Button variant="destructive" className="w-full" onClick={() => deleteExercise(detailExercise.id)}>
