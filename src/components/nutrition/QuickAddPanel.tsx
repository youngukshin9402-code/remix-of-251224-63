      ...
      <Tabs defaultValue="recent" className="w-full flex-1 flex flex-col overflow-hidden">
        ...
        <TabsContent value="sets" className="mt-3">
          <ScrollArea className="h-[200px]">
            ...
            ) : mealTypeSets.length === 0 ? (
              <div className="text-center py-8">
                <Package className="w-8 h-8 mx-auto text-muted-foreground mb-2" />
                <p className="text-muted-foreground text-sm">저장된 세트가 없어요</p>
                <p className="text-xs text-muted-foreground mt-1">
                  여러 음식을 한 번에 추가하려면 세트를 만들어보세요
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                {mealTypeSets.map((set) => (
                  <div
                    key={set.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {set.foods.length}개 음식 · {set.totalCalories}kcal
                        </p>
                      </div>
                      <div className="flex items-center gap-1">
                        <Button
                        ...
                        <Button
                          size="sm"
                          onClick={() => handleAddSet(set.foods, set.id)}
                    ...
                    <div className="flex flex-wrap gap-1">
                      {set.foods.slice(0, 3).map((food, idx) => (
                        <span
                          key={idx}
